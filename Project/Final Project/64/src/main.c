...
int CurrentLevel = 1;
int HighScores[MAX_LEVELS];
int levelTimes[MAX_LEVELS];
typedef struct
{
    int x, y;
} Position;
...
void renderScoreOnWinScreen(int, int, int);
...
void ballCollision()
{
    ...
    Position playerPosition = GetPlayerPosition();
    if (ball.pos.x < (playerPosition.x * BOX + 30) + 30 &&
...
void renderWinScreen()
{
    ...
    levelTimes[CurrentLevel - 1] = TimeLeft();
    int CurrentBestScore;
    for (int i = 0; i < CurrentLevel; i++)
    {
        CurrentBestScore += 100 * levelTimes[i];
    }
    int AlwaysHighScore = loadHighScores();
    if (AlwaysHighScore < CurrentBestScore)
    {
        saveHighestScore();
        AlwaysHighScore = CurrentBestScore;
    }
    renderScoreOnWinScreen(AlwaysHighScore, 350, 150);
    renderScoreOnWinScreen(CurrentBestScore, 350, 220);
...
{
    Snoppy.lives = 3;
    SDL_Surface *winSurface = SDL_LoadBMP("../src/GameOver.bmp");
    if (winSurface == NULL)
    {
        fprintf(stderr, "Error loading win.bmp: %s\n", SDL_GetError());
        return;
    ...
    SDL_Rect destRect = {0, 0, MAP_WIDTH + BOX, MAP_HEIGHT + BOX};
    SDL_RenderCopy(render, winTexture, NULL, &destRect);
    SDL_RenderPresent(render);
...
void renderScoreOnWinScreen(int score, int org_x, int org_y)
{
    // Render the score on the win screen
    int nbrDigits = 5; // Assuming a maximum of 4 digits for the score       // Adjust the starting X position as needed
    renderScore(score, nbrDigits, org_x, org_y);
}

void renderScoreScreen()
{
    SDL_Surface *winSurface = SDL_LoadBMP("../src/scoreScreen.bmp");
    if (winSurface == NULL)
    {
        fprintf(stderr, "Error loading win.bmp: %s\n", SDL_GetError());
        return;
    ...
    SDL_RenderCopy(render, winTexture, NULL, &destRect);
    renderScoreOnWinScreen(loadHighScores(), 400, 150);
    SDL_RenderPresent(render);
...
void saveHighestScore()
{
    ...
    for (int i = 0; i < CurrentLevel; i++)
    {
        HighScores[i] = levelTimes[i] * 100;
        totalScore += HighScores[i];
    }

    // Save the total high score to a JSON file
    cJSON *jsonObject = cJSON_CreateObject();
    cJSON_AddNumberToObject(jsonObject, "TotalScore", totalScore);

    char *jsonString = cJSON_Print(jsonObject);

    FILE *file = fopen("../src/SAVE/HightScore.json", "w");
    if (file != NULL)
    {
...
int loadHighScores()
{
    ...
    FILE *file = fopen("../src/SAVE/HightScore.json", "r");
    if (file != NULL)
    {
        fseek(file, 0, SEEK_END);
        ...
        if (jsonString != NULL)
        {
            ...
            if (jsonObject != NULL)
            {
                // Retrieve the total high score
                cJSON *totalScoreJson = cJSON_GetObjectItem(jsonObject, "TotalScore");
                if (cJSON_IsNumber(totalScoreJson))
                {
                    totalScore = totalScoreJson->valueint;

                    // Distribute the total score among levels (example: equally for simplicity)
                    for (int i = 0; i < MAX_LEVELS; i++)
                    {
                        HighScores[i] = totalScore / MAX_LEVELS;
                    }
                }
